...
description = "How I use Ghostty, Git worktrees, Herdr, hunk, Tailscale, and Heeler to run, review, and remotely manage parallel coding agents."
author = "Igor Kulman"
date = "2026-09-16T05:29:12+01:00"
tags = ["AI", "Coding Agents", "Developer Tools", "Terminal", "Git"]
keywords = ["Ghostty", "Herdr", "Heeler", "Git worktrees", "Pi", "hunk", "coding agents", "Tailscale"]
...
My current setup is mostly [Ghostty](https://ghostty.org/), [Git worktrees](https://git-scm.com/docs/git-worktree), [Herdr](https://herdr.dev/), and [hunk](https://hunk.dev/). It also has the nice property that I can access the whole thing remotely, even from my iPhone.

## The stack at a glance

```text
Ghostty → Herdr → Git worktrees → Pi + hunk panes
...
## Reviewing changes next to the agent

My usual Herdr workspace is split into two columns. The left pane runs Pi as my coding harness, while the right pane runs [hunk](https://hunk.dev/), a terminal diff viewer that presents the current Git changes in a much more readable way than a plain `git diff`.

The useful part is not just having the diff visible next to the agent. I can add inline comments directly to it: point out something I want changed, question an implementation choice, or ask for a different approach. A Pi skill can pick up those comments and feed them back to the agent.

This creates a tight review loop without leaving the terminal. Pi makes a change, I review it in the adjacent pane, annotate the exact lines that need attention, and Pi continues from that feedback. I do not have to translate every observation into a separate prompt or explain where in the diff I mean.
...
- hunk to review changes and leave inline feedback for Pi
