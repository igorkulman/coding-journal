...
That was not the whole of .NET async programming. Console apps, ASP.NET Core, and custom schedulers all have their own details. But for the kind of UI code I was writing, the default felt right: stay where I started, unless I said otherwise.
...
The answers exist. The problem is having to ask them in the first place.
...
For highly concurrent systems, this can be the right trade-off. For most iOS apps, it is extra surface area around code that was already effectively single-threaded.

## The trade-off

I do not think Swift concurrency is bad. Compile-time data-race safety is a serious achievement, and I would rather have the compiler catch real isolation bugs than discover them as rare crashes in production.
...
The "Approachable Concurrency" work in Swift 6.2 is a step in the right direction. The idea — let simple apps stay simple, bring in the full model only when you need it — is the right one.

It does not remove the complexity though. You still have to learn all of it eventually; 6.2 just lets you put it off a little longer.
